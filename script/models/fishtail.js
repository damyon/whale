...
class FishTail extends Drawable {
  ...
  setRotation(gl, offset) {

    this.angle = Math.sin(offset * 0.1) / 2;
  ...
  loadShape(gl, filename) {
    ...
    image.onload = function() {
      ...
      let row = 0, splat = 0;
      let heightOffset = this.fat, clipLimit = 0.1, clampLimit = 0.2, inverse = 1;
      ...
      for (k = 0; k < 2; k++) {
        for (i = this.LOD - 1; i >= 0; i--) {
          for (j = this.LOD - 1; j >= 0; j--) {
            ...
            if (offsetZ1 * heightOffset > clipLimit) {
              if (offsetZ1 * heightOffset < clampLimit) {
            ...
            if (offsetZ2 * heightOffset > clipLimit) {
              if (offsetZ2 * heightOffset < clampLimit) {
            ...
            if (offsetZ3 * heightOffset > clipLimit) {
              if (offsetZ3 * heightOffset < clampLimit) {
                this.positions[offset++] = offsetX + unit - 6;
                this.positions[offset++] = offsetY + unit;
                this.positions[offset++] = 0.01*inverse;
              } else {  
            ...
            if (offsetZ4 * heightOffset > clipLimit) {
              if (offsetZ4 * heightOffset < clampLimit) {
                this.positions[offset++] = offsetX - 6;
                this.positions[offset++] = offsetY + unit;
                this.positions[offset++] = 0.01*inverse;
              } else {  
